(function() {
	...
	$(function() {
		...
		/*Create Pages of 12 Thumbs*/	
		function createThumbPages(){
			var $imgItemArr = container.find('.img-item').parent('.item-wrap'),
				imgItemArrLength = $imgItemArr.length,
				itemsPerDiv = 12;
			for ( var i = 0; i < imgItemArrLength; i += itemsPerDiv ){
				$imgItemArr.filter(':eq('+i+'),:lt('+(i+itemsPerDiv)+'):gt('+i+')').wrapAll('<div class="thumb-page" />');
			}
			if( totalThumbNum < 12){
		...
		function initSlider(itemIndex){
			...
			container.find('.img-item').each(function(){
				gallery.find('.slides').append('<div class="gallery-item" data-bgsrc="' + $(this).data('large') + '" data-caption="' + $(this).data('caption') + '" data-fullscreen="' + $(this).data('fullscreen') + '" data-highres="' + $(this).data('highres') + '"></div>');
			});
			...
			/*Init Slider*/
			gallery
				.after('<span class="prev-btn" data-sobjectid="PhotoGallery:MainImage:Arrows:Left"></span>')
				.after('<span class="next-btn" data-sobjectid="PhotoGallery:MainImage:Arrows:Right"></span>')
				...
				before: function() {
					...
					if( !prev.hasClass('gallery-item') ){
						prev = gallery.find('.gallery-item').last();
						container.find('.prev-btn').hide();
					}else{
						container.find('.prev-btn').show();
					}
					...
					if( visiblePage == (currentPageIndex - 1) ){
					...
					if( $(this).index() == (container.find('.img-item').length ) ){
						container.find('.related-bar').show(0,function(){
							$(this).find('.related-galleries').slideDown();
						});
						container.find('.info-bar .caption').stop(true,true).fadeOut();
						container.find('.related-bar .related').addClass('related-open').text('Close');
					}else{
						container.find('.zoom-in').stop(true,true).fadeOut();
						container.find('.info-bar .caption').fadeIn(function(){
							container.find('.related-bar').hide();
						});
						container.find('.related-bar .related-galleries').slideUp();
						container.find('.related-bar .related').removeClass('related-open').text('View Related Galleries');					
					}
		...
		function loadAds(){		
			...
			$('.dart_ad').each(function() {
				...
				iframe.removeAttr('style').attr('src', '');
		...
		container.on('click', '.related-bar .related', function(){
			if( $(this).hasClass('related-open') ){
				...
				container.find('.related-bar .related-galleries').slideUp();
				$(this).removeClass('related-open').text('View Related Galleries');
		...
		function prevThumbPage(currentPage){
			...
			if( currentPage.prev().hasClass('thumb-page') ){
				if (agentID) {
					/*apple code*/
					currentPage.hide(300, function(){
						currentPage.prev().show(300);
						prevInner(currentPage, currentFirst, currentSecond);
					});
				}else{
					currentPage.hide('slide', {direction: 'right'}, 300, function(){
		...
		function nextThumbPage(currentPage){
			...
			if( currentPage.next().hasClass('thumb-page') ){
				if (agentID) {
					...
					currentPage.hide('slide', {direction: 'left'}, 300, function(){
		...
		function Pager() {
			this.showPage = function(pageNumber) {
				...
				var from = (pageNumber - 1) * container + 1;
				var to = from + container - 1;
			} 
		...
		/*container.find('.thumbPagination').on('click', 'li', function(){
			if( loadingThumbs == false){
				...
				if(currentEq<thisEq){
					var currentPage = $('.thumb-page.current-page').prev();
					nextThumbPage( currentPage );
					scrollToTop = false;
					clickThumb(container.find('.current-page').find('.img-item').first());
				}else{
					var currentPage = $('.thumb-page.current-page').next();
					prevThumbPage( currentPage );
					scrollToTop = false;
					clickThumb(container.find('.current-page').find('.img-item').first());
				}
		...
		function clickThumb(item){
			var itemPageIndex = item.parents('.thumb-page').index(),
				itemIndex = item.parent('.item-wrap').index() + ( itemPageIndex * 12 ),
				topPosition = $('.photos-subhead').offset().top-116;
		...
		/*	FIRST THUMBNAIL REMOVE LEFT MARGIN	*/
		$('.thumb-page > span:nth-child(4n)').css('margin-right','0px');
