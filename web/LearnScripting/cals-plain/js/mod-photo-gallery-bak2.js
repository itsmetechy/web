var imgPushUrl = "chevrolet/camaro/2012/photos/exterior/hr-2/t3-12-8/";
	...
	var	imggroupId	= $(".view-container select").find("option").val();	
			
(function() {

	$(function() {
		
		$(".view-container select").change(function() {
			imggroupId = $(this).val();
			createArticleThumbPages()
			//loadThumbPages()
			initPager();
			$('.thumb-page:first .item-wrap:first .img-item').click();
		});
		...
		function createArticleThumbPages(){
			$('.thumb-page').each(function(){
				$(this).children().unwrap();
				/*if($(this).parent().is(".item-wrap")){
					$(this).unwrap();
				}*/
			});
			//alert("sf");
			var $imgItemArr = container.find("a[data-imggroup='"+imggroupId+"']").parent('.item-wrap'),
				imgItemArrLength = $imgItemArr.length,
				itemsPerDiv = 12;
			for ( var i = 0; i < imgItemArrLength; i += itemsPerDiv ){
				$imgItemArr.filter(':eq('+i+'),:lt('+(i+itemsPerDiv)+'):gt('+i+')').wrapAll('<div class="thumb-page" />');
		...
		function initSlider(itemIndex){
			...
			gallery
				...
				before: function() {
					...
					if( $(this).index() == (container.find('.img-item').length ) ){
						...
						container.find('.related-bar .related').removeClass('related-open').text('Related Articles');					
					}*/
				...
				onPrevNextEvent: function(isNext) {
					if(isNext){
						//Next Tracking Code
						setTimeout('$(".gallery-item").mouseenter()',500)
			...
			setTimeout(function(){	
				$('.item-wrap a[data-pushurl*="'+imgPushUrl+'"]').click();
		...
		$('.thumb-page').each(function(e){
			e = e+1
			$('#thumbPagination').append('<li class="mod-button"><div class="butn btnblock"><div class="btn-inner">'+e+'</div></div></li>');
		});
