$(document).ready(function () {
    ...
    function setPagination(){
        ...
        var activeLastIndex = $lis.index($lis.filter('.active:last')).length?;
        var activeFirstIndex = $lis.index($lis.filter('.active:first'));
        
        if(activeLastIndex){
            var $toHighlight = $lis.slice(activeLastIndex + 1, activeLastIndex + 6);
